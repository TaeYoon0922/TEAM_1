...
PREDICTIONS_PATH = PROJECT_ROOT / "data" / "processed" / "role02_eval_predictions.csv"
METRICS_PATH = PROJECT_ROOT / "data" / "processed" / "role02_eval_metrics.csv"
...
def main() -> None:
    ...
    print(f"[ROLE02 평가] 라벨 CSV: {LABELED_SENTENCE_PATH.relative_to(PROJECT_ROOT)}")
...
def plot_metric_bars(metric_rows: list[dict], path: Path) -> None:
    ...
    ax.set_ylim(0, 1.1)
    ax.set_ylabel("score")
    ax.set_title("ROLE02 Precision / Recall / F1")
    ax.set_xticks(x)
...
def plot_confusion_matrices(metric_rows: list[dict], path: Path) -> None:
    ...
    plt.suptitle("ROLE02 Confusion Matrices", fontsize=14)
    plt.tight_layout()
    plt.savefig(path, dpi=150)
    plt.close()
