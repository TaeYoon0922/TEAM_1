"""질문 군집화(SBERT + KMeans, k=6) 모델 평가 + 시각화.

비지도 군집화이므로 "정답 라벨"이 없어도 성립하는 내재적(intrinsic) 평가 지표만 사용한다.
  1) 실루엣 점수(silhouette score) — k=2..10 비교로 "왜 k=6을 선택했는가"의 근거
  2) PCA 2D 산점도 — SBERT 임베딩이 실제로 군집별로 분리되는지 시각적 검증
  3) 군집 크기 분포 — 특정 군집으로 쏠리지 않고 합리적으로 나뉘었는지 확인

모두 운영 중인 캐시(cache/train_sbert_embeddings.npy, train_kmeans_model.pkl)를
그대로 사용하므로, 평가 대상과 운영 모델이 항상 같다(평가용 합성 데이터에 의존하지 않음).
"""
...
SILHOUETTE_FIGURE_PATH = FIGURE_DIR / "role05_cluster_silhouette_by_k.png"
PCA_SCATTER_FIGURE_PATH = FIGURE_DIR / "role05_cluster_pca_scatter.png"
CLUSTER_SIZE_FIGURE_PATH = FIGURE_DIR / "role05_cluster_sizes.png"

K_RANGE = range(2, 11)
CHOSEN_K = 6
SAMPLE_SIZE = 3000  # 실루엣 점수·산점도 계산용 표본 크기 (O(n^2) 비용 절감)
RANDOM_STATE = 42
...
def main() -> None:
    ...
    print(f"[ROLE05 군집화 평가] 임베딩 shape={embeddings.shape}, k={km.n_clusters}, 표본 수={len(labels)}")
...
def compute_silhouette_by_k(embeddings: np.ndarray, cached_label_for_k: dict[int, np.ndarray]) -> dict[int, float]:
    """k=2..10 각각에 대해 실제 운영과 동일한 설정(n_init=10, seed=42)으로 KMeans를 적합하고
    동일 표본(seed 고정)에 대한 실루엣 점수를 계산한다. 운영 중인 k=6 모델은 캐시된 라벨을 그대로 쓴다."""
