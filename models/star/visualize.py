...
def plot_star_summary(head_binary: dict, star_result: dict, numbers: dict, output_path: str | Path) -> Path:
    ...
    ax.set_ylim(0, 110)
    ax.set_ylabel("score")
    ax.set_title("STAR summary" if not has_korean_font else "STAR 분석 요약")
...
def plot_star_requirement_status(
    head_binary: dict,
    star_result: dict,
    numbers: dict,
    ...
    ax.set_xlim(0, 1)
    ax.invert_yaxis()
    ax.set_title("STAR requirement status" if not has_korean_font else "STAR 모델 충족 여부")
