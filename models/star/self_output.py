...
from models.star.star_scorer import score_star_completeness


SCHEMA_VERSION = "star_to_self.v1"
STAR_WEIGHTS = {
    "핵심 주장 명확성": 0.4,
    "경험 구체성": 0.6,
}
...
def build_self_payload(
    ...
    star_score = calc_star_score(core_metric["score"], experience_metric["score"])
    ...
    return {
        ...
        "star_summary": {
            "score": star_score,
            "level": level_from_score(star_score),
...
def calc_star_score(core_claim_score: int, experience_score: int) -> int:
    score = (
        core_claim_score * STAR_WEIGHTS["핵심 주장 명확성"]
        + experience_score * STAR_WEIGHTS["경험 구체성"]
    )
